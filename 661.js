"use strict";
(self["webpackChunkambire_mobile_wallet"] = self["webpackChunkambire_mobile_wallet"] || []).push([[661],{
...
function _abytes2(value, length, title = '') {
    const bytes = (0,esm_utils/* isBytes */._t)(value);
...
function utils_bytesToNumberBE(bytes) {
    return hexToNumber((0,esm_utils/* bytesToHex */.ci)(bytes));
}
function utils_bytesToNumberLE(bytes) {
    (0,esm_utils/* abytes */.gk)(bytes);
    return hexToNumber((0,esm_utils/* bytesToHex */.ci)(Uint8Array.from(bytes).reverse()));
}
function numberToBytesBE(n, len) {
    return (0,esm_utils/* hexToBytes */.nr)(n.toString(16).padStart(len * 2, '0'));
}
...
function utils_ensureBytes(title, hex, expectedLength) {
    ...
    if (typeof hex === 'string') {
        try {
            res = (0,esm_utils/* hexToBytes */.nr)(hex);
        }
        catch (e) {
            throw new Error(title + ' must be hex string or Uint8Array, cause: ' + e);
        }
    }
    else if ((0,esm_utils/* isBytes */._t)(hex)) {
...
function createHmacDrbg(hashLen, qByteLen, hmacFn) {
    ...
    const gen = () => {
        ...
        return (0,esm_utils/* concatBytes */.eV)(...out);
    };
...
const validatorFns = {
    ...
    stringOrUint8Array: (val) => typeof val === 'string' || (0,esm_utils/* isBytes */._t)(val),
...
function modular_nLength(n, nBitLength) {
    ...
    if (nBitLength !== undefined)
        (0,esm_utils/* anumber */.k8)(nBitLength);
...
function weierstrassN(params, extraOpts = {}) {
    ...
    function pointToBytes(_c, point, isCompressed) {
        ...
        if (isCompressed) {
            ...
            return (0,esm_utils/* concatBytes */.eV)(pprefix(hasEvenY), bx);
        }
        else {
            return (0,esm_utils/* concatBytes */.eV)(Uint8Array.of(0x04), bx, Fp.toBytes(y));
        }
    }
    ...
    class Point {
        ...
        toHex(isCompressed = true) {
            return (0,esm_utils/* bytesToHex */.ci)(this.toBytes(isCompressed));
        }
        toString() {
...
function ecdh(Point, ecdhOpts = {}) {
    const { Fn } = Point;
    const randomBytes_ = ecdhOpts.randomBytes || esm_utils/* randomBytes */.O6;
...
function ecdsa(Point, hash, ecdsaOpts = {}) {
    ...
    const randomBytes = ecdsaOpts.randomBytes || esm_utils/* randomBytes */.O6;
    const hmac = ecdsaOpts.hmac ||
        ((key, ...msgs) => (0,esm_hmac/* hmac */.b)(hash, key, (0,esm_utils/* concatBytes */.eV)(...msgs)));
    const { Fp, Fn } = Point;
    ...
    class Signature {
        ...
        static fromHex(hex, format) {
            return this.fromBytes((0,esm_utils/* hexToBytes */.nr)(hex), format);
        }
        ...
        recoverPublicKey(messageHash) {
            ...
            const R = Point.fromBytes((0,esm_utils/* concatBytes */.eV)(pprefix((rec & 1) === 0), x));
        ...
        toBytes(format = defaultSigOpts_format) {
            validateSigFormat(format);
            if (format === 'der')
                return (0,esm_utils/* hexToBytes */.nr)(DER.hexFromSig(this));
            const r = Fn.toBytes(this.r);
            const s = Fn.toBytes(this.s);
            if (format === 'recovered') {
                if (this.recovery == null)
                    throw new Error('recovery bit must be present');
                return (0,esm_utils/* concatBytes */.eV)(Uint8Array.of(this.recovery), r, s);
            }
            return (0,esm_utils/* concatBytes */.eV)(r, s);
        }
        toHex(format) {
            return (0,esm_utils/* bytesToHex */.ci)(this.toBytes(format));
        }
        // TODO: remove
        assertValidity() { }
        ...
        toDERHex() {
            return (0,esm_utils/* bytesToHex */.ci)(this.toBytes('der'));
        }
        toCompactRawBytes() {
            return this.toBytes('compact');
        }
        toCompactHex() {
            return (0,esm_utils/* bytesToHex */.ci)(this.toBytes('compact'));
        }
    }
    ...
    function prepSig(message, privateKey, opts) {
        ...
        const seed = (0,esm_utils/* concatBytes */.eV)(...seedArgs); // Step D of RFC6979 3.2
    ...
    function tryParsingSig(sg) {
        ...
        let sig = undefined;
        const isHex = typeof sg === 'string' || (0,esm_utils/* isBytes */._t)(sg);
        const isObj = !isHex &&
            sg !== null &&
            typeof sg === 'object' &&
            typeof sg.r === 'bigint' &&
...
const p256 = createCurve({ ...p256_CURVE, Fp: Fp256, lowS: false }, sha2/* sha256 */.JQ);
...
const p384 = createCurve({ ...p384_CURVE, Fp: Fp384, lowS: false }, sha2/* sha384 */.iC);
...
const p521 = createCurve({ ...p521_CURVE, Fp: Fp521, lowS: false, allowedPrivateKeyLengths: [130, 131, 132] }, sha2/* sha512 */.o);
